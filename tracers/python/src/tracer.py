...
class Tracer:
    ...
    def run(self):
        ...
        try:
            ...
            if not isinstance(e, UserWarning):
                ...
                self._result.append({'threw': {'exception': exceptionMessage(e, remove_lines=(1,))}})
        finally:
            sys.settrace(None)
    ...
    def _trace(self, frame: types.FrameType, event: str, args):
        ...
        if self._current_step > self._steps:
            self._result.append({'threw': {'cause': f'maximum steps: {self._steps}'}})
            raise UserWarning()
        if self._missing_input:
            self._result.append({'threw': {'cause': 'not enough input'}})
            raise UserWarning()
