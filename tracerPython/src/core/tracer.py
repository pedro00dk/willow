...
class Tracer:
    ...
    def run(self):
        try:
            ...
            if not isinstance(e, UserWarning): # UserWarning are generated by user stop actions
...
class FrameProcessor:
    ...
    def trace(self, frame: types.FrameType, event: str, args):
        ...
        while True:
            action = self._action_queue.get()
            if action.name == message.Action.STOP:
                # the only way to stop any traced code is raising exceptions
    ...
    def input_hook(self, prompt: str):
        ...
        while True:
            ...
            action = self._action_queue.get()
            if action.name == message.Action.STOP:
                # add stop message in the queue again for stacked inputs until reach trace function
